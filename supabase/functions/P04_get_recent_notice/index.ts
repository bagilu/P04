...
serve(async (req) => {
  ...
  try {
    ...
    const afterCreatedAt = String(body?.after_created_at ?? "").trim();
    ...
    const rows = (data ?? []).map((row: any) => ({
      ...
      responder_nickname: row.responder_nickname || row.responder_account || "某位同學",
      smile_type: row.smile_type,
      smile_type_label: smileTypeLabel(row.smile_type),
    }));

    return jsonResponse({ success: true, rows, server_time: new Date().toISOString() });
  } catch (error) {
